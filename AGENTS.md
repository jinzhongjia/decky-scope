...
On 2026-09-12 at 20:58 the user explicitly required maintainer-only commit attribution. Use `jinzhongjia <mail@nvimer.org>` for both author and committer. Do not add `Co-authored-by` trailers, collaborator credits, or AI/bot attribution to commit messages. This supersedes the earlier collaborator-trailer convention. Preserve the current rewritten branch history; do not restore old commit IDs or rewrite published history unless explicitly requested.
