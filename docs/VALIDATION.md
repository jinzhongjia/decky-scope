...
> **当前版本为 `0.1.0-rc.2`，已改为 QAM-only。** 最新结果：22 项 Zig、26 项 Python（两种构建分别通过）、12 项前端测试，以及实际 QAM 图表、系统信息、设置、焦点和恢复验收。rc.2 产物身份与输入结果见 [原生 UI 输入验收](UI-INPUT-ACCEPTANCE.md)，原 QAM 基础功能范围见 [rc.1 验收](QAM-ACCEPTANCE.md)。触摸原始问题仍未稳定复现。以下保留 02:47 以前版本的历史基线，旧 UI 描述及哈希不适用于当前候选包。
